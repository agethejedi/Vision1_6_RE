...
const GCONF = (window.VisionConfig && window.VisionConfig.GRAPH) || {};
const RXL_FLAGS = Object.freeze({
  debounceViewportMs: GCONF.debounceViewportMs || 180
});
...
async function init() {
  ...
  await post('INIT', {
    ...
    network: getNetwork(),
    ruleset: window.VisionConfig?.RULESET || 'safesend-2025.11.0',
    concurrency: 8,
    flags: {
      graphSignals: true,
      streamBatch: true,
...
function bindUI() {
  ...
  document.getElementById('resetView')?.addEventListener('click', () => {
    if (window.graph && typeof window.graph.resetView === 'function') {
...
function setSelected(id) {
  selectedNodeId = id ? normId(id) : null;
}
...
function pushNav(id){
  if (!id) return;
  if (navIndex >= 0 && navHistory[navIndex] === id) return;
...
function setNeighborStatus(text){
  const el = document.getElementById('neighborStatus');
  if (el) el.textContent = text;
}
function setNeighborStatusLoading(){
  const el = document.getElementById('neighborStatus');
  if (!el) return;
  el.innerHTML = `<span class="spinner"></span> <span>Loading neighbors…</span>`;
}
...
/* ================= Neighbors (via worker → /neighbors) ========== */

async function getNeighborsLive(centerId){
  try {
    const res = await post('NEIGHBORS', {
      id: centerId,
      network: getNetwork(),
      hop: 1,
...
async function refreshGraphFromLive(centerId){
  ...
  if (!nodes.length && !links.length) {
    setNeighborStatus('Neighbors: sparse or unavailable');
    return;
  }
  ...
  const ll = links.map(L => ({
    a: normId(L.a || L.source || L.idA || L.from),
    b: normId(L.b || L.target || L.idB || L.to),
    weight: L.weight || 1
  }));
  ...
  for (const n of nn) {
    if (!knownNeighbors.has(n.id) && n.id !== center.id) {
      ll.push({ a: center.id, b: n.id, weight: 1 });
    }
  }
  ...
  for (const n of finalNodes) {
    if (n.id !== center.id) {
      window.graph?.setHalo({ id: n.id, color:'#22d37b', intensity:.35 });
    }
  }
  window.graph?.setHalo({ id: center.id, intensity:.95 });

  setNeighborStatus(`Neighbors: ${finalNodes.length - 1} nodes`);
}
...
function narrativeFromExplain(expl, mode = 'analyst') {
  ...
  if (lastRenderResult?.sparseNeighborhood) {
    text += ' Limited neighbor data — metrics may be conservative.';
  }
